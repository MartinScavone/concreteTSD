...
%   pass to the TSD_denoisingJoints function!
 ...
 %%Pass data for the plot: T vs the steinError and locate the optimum T value (that who %minimizes the SURE)
    ...
    SUREPlotNumber = 200+k;
    SUREPlotName   = sprintf('Denoised TSD - %s, section %s : optimum t value location',string(thatRoadName), string(whichRoad));
    
    [denoisedTSD,~,ySpikes,optLambda,optSURE,SUREPlotHandle] = TSD_denoisingJoints(refTSD,callType,0,SUREPlotNumber,SUREPlotName);
    ...
    grid on
    title('TSD signal denoised with BPD and soft-shrinking threshold')
    xlabel ('station [m]')
    ylabel ('D_0')
    hold on
    ...
    fprintf(' The optimum value for the threshold t is %g \n',optLambda);
    fprintf(' The corresponding SURE estimate is %g \n',optSURE);
...
exportFilename = 'NatlMall_weakSpotChart_V2022-0501.xlsx';
